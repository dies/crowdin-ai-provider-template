...
const app = crowdinModule.express(); // The Express instance provided by the Crowdin framework implements some API for best application performance
...
const configuration = {
    ...
    aiProvider: {
        ...
        getModelsList: async ({ client, context }) => {
            ...
            const formData = await crowdinApp.getMetadata(`form-data-${context.jwtPayload.context.organization_id}`);
            ...
            return (modelsResponse?.data || [])
                ...
                .map(model => ({ //TODO: parse from model if available
                    id: model.id.trim(),
        ...
        chatCompletions: async ({ messages, model, action, responseFormat, client, context, req }) => {
            const formData = await crowdinApp.getMetadata(`form-data-${context.jwtPayload.context.organization_id}`);
...
app.post(
    '/form',
    crowdinModule.postRequestCredentialsMasker(require('./settings-form').getForm()), // mask the credentials
    async (req, res) => {
        ...
        try {
            ...
            await crowdinApp.saveMetadata(`form-data-${context.jwtPayload.context.organization_id}`, formData);
            res.status(200).send({ message: 'Credentials are valid. The integration is ready to use.' });
        } catch (e) {
            console.error(e);
            res.status(400).send({
...
if (process.env.DB_HOST) {
    ...
    app.listen(process.env.PORT || 3000, () => console.info(`Crowdin app listening on port ${process.env.PORT || 3000}`));
}